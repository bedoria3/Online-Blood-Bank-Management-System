...
<body>
  ...
  <section class="home-section">
    ...
    $id = $_GET['id'];
    $qry = "select * from blooddetails where id='$id'";
    ...
    <div id="sample">
      <form action="editedblood.php" method="post" style="margin-left: 30%;">
        <label for="fname">BloodType Name</label>
        <input type="text" id="btype" name="btype" placeholder="Blood Group"
          value='<?php echo $row['BloodType']; ?>' required>

        <label for="calendar">Creation Date</label>
        <input type="date" name="cdate" placeholder="creation Date.." value='<?php echo $row['creationdate']; ?>'
          required>

        <label for="des">Description</label> <br>
        <textarea id="desc" name="desc" rows="4" cols="50" required><?php echo $row['description']; ?>
</textarea>


        <input type="hidden" name="id" value="<?php echo $row['id']; ?>" <div> <br>
